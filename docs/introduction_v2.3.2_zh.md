# Oracle AI 数据库记忆系统 v2.3.2

**版本**: v2.3.2 | **日期**: 2026-05-27 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v2.3.2 新增 Web UI 优化（分页、粘性表头、视口修复）；v2.3.1 修复向量检索能力，新增五信号融合检索、全文搜索和统一搜索 API，与 v2.3.0 数据库向后兼容。
...
纯前端改进，无数据库或 API 变更。所有 183 项测试继续通过。

| 改进 | 说明 |
|------|------|
| 客户端分页 | PAGE_SIZE=30，Prev/Next + 页码按钮；7个数据页使用单分页，Agents 页使用三组独立分页（registry/sessions/collabs） |
| 粘性表头 | `position:sticky;top:0;z-index:2;box-shadow` 滚动时表头固定 |
| 视口高度修正 | `height:100vh` 防止布局溢出；内容区域 `calc(100vh - 120px)` |
| 表格间距 | `border-collapse:separate;border-spacing:0` 统一单元格渲染 |
| 登录语言持久化 | 语言偏好保存到 `localStorage`，页面加载时恢复 |
...
```
Oracle Memory System v2.3.2 - 完整测试套件
============================================================
  连接:         6/6 通过
  记忆:         8/8 通过
  知识:         8/8 通过
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **v2.3.2** | 2026-05-26 | 向量检索修复、五信号融合检索、全文搜索、统一搜索API |
