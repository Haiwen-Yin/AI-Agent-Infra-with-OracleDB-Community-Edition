# AI Agent Infra with OracleDB — 社区版 v3.4.0

**版本**: v3.4.0 | **日期**: 2026-06-08 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.4.0 引入 **Oracle Deep Data Security 深度数据安全**，构建 5+1 层数据库访问安全模型（规范约束、最小权限用户、AUTHID DEFINER、Deep Data Security 深度数据安全、统一审计、凭证脱敏），替代已废弃的 VPD 行级安全。同时完善 Multi-Agent Collaboration 多 Agent 协同的五层联动模型（Spec→协作组→Branch→Task Plan→Harness）
...
在持续演进过程中，项目逐步扩展了 Agent 管理、工作空间、规格驱动开发、Skill 分发、身份认证、上下文分支等能力，从单一的记忆系统成长为覆盖 AI Agent 全生命周期的基础设施架构。v3.4.0 引入 Deep Data Security 替代 VPD：

- **数据库访问安全**：5+1 层防护模型，从规范约束到技术审计的纵深防御
- **Deep Data Security 深度数据安全**：Data Grants 声明式行/列/单元级访问控制 + MAC 强制访问控制 + 零信任安全模型（v3.3.0 VPD 已废弃）
- **Portal agent context 数据隔离**：Portal 用户自动绑定 Pool Agent context，SYS_CONTEXT 驱动行级过滤
...
| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.4.0** | 2026-06-08 | Oracle Deep Data Security 深度数据安全、Data Grants、MAC、End User Context、零信任安全模型、凭证脱敏、Portal agent context 数据隔离 |
...
> **L4 Deep Data Security 执行状态（v3.4.0）**
>
...
> - 20 个 Data Grants（含 Portal 写操作所需的 INSERT/UPDATE 授权）
>
