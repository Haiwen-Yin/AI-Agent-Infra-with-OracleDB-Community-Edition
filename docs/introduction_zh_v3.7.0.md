# AI Agent Infra with OracleDB — 社区版 v3.7.0

**版本**: v3.7.0 | **日期**: 2026-06-18 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.6.0 引入 Admin/Agent 分离架构：新增模式系统（standalone/admin/agent），将 Admin Agent（运行 Web Portal、持有 AIADMIN 凭证）与 Business Agent（独立进程、仅持有 End User 凭证）分离。新增 Admin Token 认证、加密凭证分发、Agent Bootstrap CLI、模式感知 connection.py，消除 Business Agent 对 AIADMIN 凭证的需求。v3.7.0 修复 Portal 登录缺失方法、文档不一致等问题。
...
- **文档修正**：Deep Sec 引入版本号 v3.5.0→v3.4.0、PBKDF2 描述 SHA256/100K→SHA512/210K、Admin Token 格式 AT_+32hex、Data Grant 数量 22→23、测试计数更新、端口号 8000→18080/18090、版本号显示统一至 v3.7.0
...
| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.7.0** | 2026-06-18 | Bug fix：Portal 登录修复、图谱交互改进、文档一致性修正 |
...
> **L4 Deep Data Security 执行状态（v3.7.0）**
>
...
## 七·五、循环工程（Loop Engineering）[NEW v3.7.0]
