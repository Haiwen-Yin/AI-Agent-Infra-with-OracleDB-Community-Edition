# AI Agent Infra with OracleDB — 社区版 v3.6.0

**版本**: v3.6.0 | **日期**: 2026-06-13 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.6.0 引入 Admin/Agent 分离架构：新增模式系统（standalone/admin/agent），将 Admin Agent（运行 Web Portal、持有 AIADMIN 凭证）与 Business Agent（独立进程、仅持有 End User 凭证）分离。新增 Admin Token 认证、加密凭证分发、Agent Bootstrap CLI、模式感知 connection.py，消除 Business Agent 对 AIADMIN 凭证的需求。
...
| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.6.0** | 2026-06-13 | Admin/Agent 分离架构：模式系统（standalone/admin/agent）、Admin Token 认证、加密凭证分发、Agent Bootstrap CLI、模式感知 connection.py |
...
> **L4 Deep Data Security 执行状态（v3.6.0）**
>
