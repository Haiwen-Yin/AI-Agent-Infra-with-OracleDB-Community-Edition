# AI Agent Infra with OracleDB — 社区版 v3.7.1

**版本**: v3.7.1 | **日期**: 2026-06-19 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.7.0 引入循环工程（Loop Engineering）——第4代 AI 工程方法论（继提示词工程、上下文工程、Harness 工程之后），由 Peter Steinberger 于2026年6月提出。新增4张循环表、LOOP_MANAGER PL/SQL包、loop_api.py Python模块、6种评估类型、生命周期钩子和3个调度作业。v3.7.1 新增循环工程协同集成：规格驱动循环、Task-Loop 绑定、协作循环、SPEC_VALIDATION 与 AGGREGATE 评估类型。同时修复会话持久化、PG 循环 API 兼容性等问题。

### v3.6.0 新增功能

**Admin/Agent 分离架构**：三模式运行（standalone/admin/agent），Admin Agent 运行 Web Portal 持有 AIADMIN 凭证，Business Agent 独立进程仅持有 End User 凭证。

**Admin Token 认证**：`AT_` + 32hex 格式令牌，DB_CRYPTO 加密存储，常量时间验证，可轮换。

**加密凭证分发**：PBKDF2-HMAC-SHA512（210000 迭代）派生密钥加密 End User 凭证，每次注册使用唯一盐值。

**Agent Bootstrap CLI**：`agent_bootstrap.py` 支持 register/recover/test/skill-list/skill-acquire/skill-create/skill-update/skill-delete 命令。

**Recovery Code（恢复码）**：Agent 注册时生成 8 个一次性恢复码（RC-XXXX-XXXX-XXXX），SHA-256 哈希后 DB_CRYPTO 加密存储于 SYSTEM_CONFIG。恢复码仅注册时返回一次，Agent 必须安全保存。

**Agent 恢复流程**：
1. 调用 `POST /api/admin/agent/recover`（需 admin_token + recovery_code）
2. 验证恢复码有效且未使用
3. 检查 LAST_SEEN_AT（5 分钟窗口，可能在线则拒绝）
4. **重置 End User 密码** → 旧密码立即失效 → 旧进程无法重连 DB → 杜绝双活
5. 返回新加密凭证

**私有 Skill 备份**：Skill 设置 `visibility=PRIVATE` + `owned_by_agent=agent_id`，仅该 Agent 可见/可获取。Data Grant 谓词天然隔离 End User，Admin API 层面增加 agent_id 过滤。

**Admin Skill 管理**：Business Agent 通过 Admin API 完整管理 Skill（创建/更新/删除/上传），无需直接访问 SKILL_META 表。
...
- **文档修正**：Deep Sec 引入版本号 v3.5.0→v3.4.0、PBKDF2 描述 SHA256/100K→SHA512/210K、Admin Token 格式 AT_+32hex、Data Grant 数量 22→23、测试计数更新、端口号 8000→18080/18090、版本号显示统一至 v3.7.1
...
在持续演进过程中，项目逐步扩展了 Agent 管理、工作空间、规格驱动开发、Skill 分发、身份认证、上下文分支等能力，从单一的记忆系统成长为覆盖 AI Agent 全生命周期的基础设施架构。v3.6.0 引入 Admin/Agent 分离架构：
...
- **Admin/Agent 分离**：新增模式系统（standalone/admin/agent），Admin Agent 运行 Web Portal 并持有 AIADMIN 凭证，Business Agent 以独立进程运行仅持有 End User 凭证
- **Admin Token 认证**：`generate_admin_token()` / `verify_admin_token()` 用于 Business Agent 安全注册
- **加密凭证分发**：使用 admin_token 作为密钥材料，通过 PBKDF2 加密分发 End User 凭证
- **Agent Bootstrap CLI**：`agent_bootstrap.py` 命令行工具，自动完成 Business Agent 注册与配置
- **模式感知连接**：Agent 模式下 connection.py 不初始化 AIADMIN 连接池，仅使用 End User 连接

### 版本演进

| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.7.1** | 2026-06-19 | 循环工程协同集成：Spec-Driven Loop、Task-Loop Binding、Collaborative Loop、SPEC_VALIDATION/AGGREGATE 评估类型、Skill-Triggered Loop；会话持久化与认证修复 |
...
| **v3.6.0** | 2026-06-13 | Deep Sec 多 Agent 协同修复：ENTITIES_AGENT_OWN 谓词补全 COLLAB 子查询、新增 2 个 COLLAB Data Grant（collab_member_own / collab_group_member_access）、Data Grant 22→23 |
| **v3.5.0** | 2026-06-11 | SHARED 实体可见性修复、COLLAB 表访问修复、VISIBILITY 隔离 |
| **v3.4.0** | 2026-06-11 | Oracle Deep Data Security 深度数据安全、Data Grants、MAC、End User Context、零信任安全模型、凭证脱敏、Portal agent context 数据隔离 |
...
│  │ Bootstrap│  │ 连接池   │  │ .json（加密） │  │
│  └──────────┘  └──────────┘  └───────────────┘  │
│  ✗ 无 AIADMIN   ✓ Data Grant 强制执行  	      │
└─────────────────────────────────────────────────┘
```
...
> **L4 Deep Data Security 执行状态（v3.7.1）**
>
> Deep Sec 通过 **Direct Logon with Local End Users** 模式在数据库层完全执行：
> - 每个 Pool Agent 对应一个 Deep Sec End User（名称 = `UPPER(REPLACE(agent_id, '-', '_'))`）
...
> - 无需外部 IAM、TCPS 或 token — 使用 Oracle Direct Logon 模式
> - `connection.py` 自动路由：`set_agent_context()` → End User 连接；无上下文 → AIADMIN 池
> - `agent_api.register_agent()` 自动创建 End User
> - `END_USER_MANAGER` PL/SQL 包管理 End User 生命周期
> - 每请求上下文隔离：`_set_context_from_session()` 确保 Admin/Portal 请求使用正确的数据库身份
> - 23 个 Data Grants（含 Portal 写操作所需的 INSERT/UPDATE 授权，以及 collab_member_own / collab_group_member_access 协作组访问授权）
>
...
>
> E2E 修复（v3.7.1 测试中发现）：
> - SHARED 实体对 End User 不可见：ENTITIES_AGENT_OWN Data Grant 谓词缺少 COLLAB 子查询，仅检查 WORKSPACES.CURRENT_AGENT_ID（多数工作区为 NULL），忽略协作组成员身份。修复：添加 UNION 与 COLLAB_GROUPS + COLLAB_GROUP_MEMBERS 子查询，匹配 BRANCH_AGENT_ACCESS 等已有模式
> - End User 无法访问 COLLAB 表：WORKSPACES、WORKSPACE_CONTEXT、CONTEXT_BRANCHES、TASK_PLANS 的 Data Grant 谓词在子查询中引用 COLLAB_GROUPS 和 COLLAB_GROUP_MEMBERS，但这两个表没有 Data Grant。End User 子查询执行静默失败（ORA-00942），导致整个谓词返回 FALSE。修复：新增 `collab_member_own`（COLLAB_GROUP_MEMBERS，WHERE agent 匹配）和 `collab_group_member_access`（COLLAB_GROUPS，WHERE group_id 在成员组中）Data Grant
> - WORKSPACE_CONTEXT 协作隔离缺失：WORKSPACE_CONTEXT 表缺少 VISIBILITY 列，协作组工作区中所有上下文对所有成员可见，包括其他 Agent 的私有思考。修复：新增 VISIBILITY 列（PRIVATE/SHARED/PUBLIC，默认 SHARED），更新 WS_CTX_AGENT_ACCESS 和 WS_CTX_AGENT_INSERT Data Grant 谓词实施可见性过滤——Agent 始终可见自身上下文，可见同协作组其他 Agent 的 SHARED/PUBLIC 上下文，无法看到其他 Agent 的 PRIVATE 上下文
>
> E2E 修复（v3.4.0 测试中发现）：
> - Portal 登录上下文时序修复：`_set_portal_agent_context()` 移至 AIADMIN 操作之后
> - 新增 `ws_ctx_agent_insert`（WORKSPACE_CONTEXT INSERT）、`ws_agent_access`（WORKSPACES SELECT）、`ws_agent_update`（WORKSPACES UPDATE）Data Grants
> - WORKSPACE_CONTEXT SELECT predicate 增加 `OR AGENT_ID = ORA_END_USER_CONTEXT.username` 兼容 INSERT "with check"
> - 全局 agent context 干扰修复：每个 HTTP 请求开始时根据 session 设置正确 context
> - Portal API End User 上下文阻塞修复：Portal API（user/profile, chat/sessions, chat/history, chat/new, chat/send, chat/rename, chat/delete, chat/switch, agent/release, user/workspaces, user/memories）访问 WORKSPACES/SYSTEM_USERS 表时，因 WORKSPACES.CURRENT_AGENT_ID 为 NULL 导致 Data Grant predicate 拒绝所有行。修复：Portal API 在需要访问这些表时临时使用 `connection.set_agent_context(None)` 切换到 AIADMIN 连接
...
## 循环工程协同集成（Loop Engineering Collaborative Integration）[NEW v3.7.1]
...
## 七·五、循环工程（Loop Engineering）[NEW v3.7.1]
...
- **Python 3.8+，需安装 `oracledb 4.0.1+`** — 4.0.0 版本存在 TCPS 协议不兼容（ORA-29019）且缺少 `create_end_user_security_context` API。安装：`pip install oracledb>=4.0.1`。
...
> ⚠️ **关键**：Python `oracledb` 必须为 **4.0.1** 或更高版本。
