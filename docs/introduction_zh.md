# AI Agent Infra with OracleDB — 社区版 v4.4.11

**版本**: v4.4.11 | **日期**: 2026-08-31 | **作者**: 尹海文 | **许可**: Apache License 2.0
...
> v4.4.7 维护优化：LLM 配置保存与逻辑退役结果可解释，保存配置探活会校验实际返回模型并回写健康状态；Dashboard 权限清单使用单次实时授权上下文计算，页面分组与间距保持一致。v4.4.6 的统一身份准入能力继续保留。
...
| **v4.4.7** | 2026-08-17 | 小规模维护优化：LLM 生命周期与模型身份探活、健康状态回写、权限清单性能和 Dashboard 布局一致性 |
| **v4.4.6** | 2026-08-16 | 统一 Human 注册与独立注册页、可配置字段和一次性 Token、Portal 会话排他、企业身份回调预留、Agent 来源筛选和 Graph 能力姿态 |
