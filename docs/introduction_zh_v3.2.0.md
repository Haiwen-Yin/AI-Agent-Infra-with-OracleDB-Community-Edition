# AI Agent Infra with OracleDB — 社区版 v3.2.0

**版本**: v3.2.0 | **日期**: 2026-06-03 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.2.0 引入 **Context Branching 上下文分支** 和 **Multi-Agent Collaboration 多 Agent 协同**，支持在工作空间内 fork/merge/abandon/resume 对话上下文分支，并实现协作组与 Branch、Spec、Task Plan、Harness 五层联动的多 Agent 协同工作流。支持单 Agent 回溯探索和多 Agent 协作分支两种核心场景。被放弃的分支保留为只读学习参照，支持手动标记和自动提取学习。
...
| 上下文审计 | 规则引擎 + embedding 语义检测、空闲/越界/泄露检测 |
...
在持续演进过程中，项目逐步扩展了 Agent 管理、工作空间、规格驱动开发、Skill 分发、身份认证、上下文分支等能力，从单一的记忆系统成长为覆盖 AI Agent 全生命周期的基础设施架构。v3.2.0 在 v3.1.0 的基础上引入上下文分支能力：

- **上下文分支**：支持工作空间内的上下文 fork/merge/abandon/resume，实现单 Agent 回溯探索和多 Agent 协作分支
- **智能合并**：自动检测实体冲突，提供冲突列表
- **学习参照**：ABANDONED 分支保留只读 + mark_as_lesson 手动标记 + extract_lessons 自动提取
- **数据模型**：CONTEXT_BRANCHES + BRANCH_MERGE_LOG + BRANCH_COMPARISON 视图

### 版本演进

| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.2.0** | 2026-06-03 | Context Branching 上下文分支、多Agent协同、智能合并、学习提取 |
| v3.1.0 | 2026-06-02 | 完全重构与重新定位，双版本策略，社区版发布，DB_CRYPTO 数据库原生加密 |
...
### 社区版（Enterprise Edition）

- **许可证**：Apache License 2.0（BSL 1.1）
...
| `audit_idle_timeout_min` | 60 分钟 | 超过此时间 → 记录 IDLE_PATTERN 审计事件（社区版） |
...
工作空间上下文审计是社区版的安全合规能力，通过规则引擎和 embedding 语义检测识别 Agent 效率问题。
...
| BRANCH_MANAGER | 9 | 上下文分支管理：fork/merge/abandon/pause/resume/diff/conflicts/lesson |
...
- Python 3.8+，需安装 `oracledb` 和 `ldap3` 包
...
**社区版**：Apache License 2.0（BSL 1.1）
