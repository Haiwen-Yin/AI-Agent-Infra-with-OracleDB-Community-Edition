# AI Agent Infra with OracleDB — 社区版 v3.5.0

**版本**: v3.5.0 | **日期**: 2026-06-11 | **作者**: 尹海文 | **许可**: Apache License 2.0

> v3.5.0 修复 Deep Sec 多 Agent 协同关键 Bug：ENTITIES_AGENT_OWN Data Grant 谓词缺少 COLLAB 子查询导致 SHARED 实体对 End User 不可见；新增 `collab_member_own` 和 `collab_group_member_access` 两个 Data Grant 解决 End User 无法访问 COLLAB 表（ORA-00942）的问题。Data Grant 总数 20 → 22。
...
在持续演进过程中，项目逐步扩展了 Agent 管理、工作空间、规格驱动开发、Skill 分发、身份认证、上下文分支等能力，从单一的记忆系统成长为覆盖 AI Agent 全生命周期的基础设施架构。v3.5.0 修复 Deep Sec 多 Agent 协同关键 Bug：

- **SHARED 实体可见性修复**：ENTITIES_AGENT_OWN Data Grant 谓词补全 COLLAB 子查询，SHARED 实体对协作组成员正确可见
- **COLLAB 表访问修复**：新增 `collab_member_own` 和 `collab_group_member_access` Data Grant，End User 可正常访问 COLLAB_GROUPS/COLLAB_GROUP_MEMBERS
- **Data Grant 总数**：20 → 22
...
| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v3.5.0** | 2026-06-11 | Deep Sec 多 Agent 协同修复：ENTITIES_AGENT_OWN 谓词补全 COLLAB 子查询、新增 2 个 COLLAB Data Grant（collab_member_own / collab_group_member_access）、Data Grant 20→22 |
...
> **L4 Deep Data Security 执行状态（v3.5.0）**
>
...
> - 22 个 Data Grants（含 Portal 写操作所需的 INSERT/UPDATE 授权，以及 collab_member_own / collab_group_member_access 协作组访问授权）
>
