...
Work continuity: [English operations guide](docs/continuity-operations.md) · [工作交接与诊断中文操作说明](docs/continuity-operations_zh.md).

Knowledge answers: [Channel and Portal boundaries](docs/channel-knowledge.md) · [频道与 Portal 知识问答](docs/channel-knowledge_zh.md).

> **v4.4.16 · Community Edition · Oracle**
>
> Database-backed AI Agent infrastructure for Oracle.

![License](https://img.shields.io/badge/License-Apache_2.0-green)

## v4.4.16 Release Highlights
...
Provider Profile may allow direct access, platform-gateway access, or both.
Changes require a compliance reason and the forwarding address is generated by
the running platform. The gateway records safe Token, provenance, cost, latency,
and correlation facts without retaining prompts or generated responses.

The management wallboard combines Agent, Session, Task Plan, Loop, and stalled
runtime state with 14-day Token and cost curves, bounded usage detail, coverage,
and freshness. Gateway observation supplements existing database-authoritative
identity, authorization, and compliance controls; direct calls outside verified
adapters remain explicitly unobserved. Effective-dated hard/warn quotas,
encrypted idempotent replay, Provider invoice reconciliation, Enterprise
chargeback, and signed external evidence are governed fact layers rather than
claims of universal traffic discovery.

Knowledge visibility is evaluated from database-authoritative policy at every
list, item, graph, and retrieval read. Company-public, organization-subtree,
organization-level, and Human/Agent-private scopes use the current organization
hierarchy; sibling departments do not gain access merely because they use the
same platform.
...
Security Domains remain the explicit authorization boundary and Channels are
the product-facing collaboration surface. Legacy collaboration groups remain
internal compatibility execution records, not permission grants.
...
Channel or run in that domain. Historical group membership and sharing policy
never become authorization automatically.
...
The v4.4.13 release added a knowledge-grounded Portal. It checks
the Human/Agent authorization intersection before retrieval, uses an approved
LLM for cited answers, and can use clearly labeled general model knowledge only
when database policy permits supplementation. Without a model, authorized
Knowledge extracts remain available. Migrations 80-81 initialize and complete six bilingual
product-knowledge topics for Oracle, PostgreSQL, and YashanDB.

The v4.4.14 release adds database-authoritative capability posture for model,
MCP, A2A, and governed execution. External MCP/A2A and model-generated writes
remain disabled or proposal-only unless current policy and Human approval permit
a governed operation. Release status is defined by the current evidence
manifest, not by this overview text.
...
├── RELEASE_NOTES_v4.4.16.md   # release notes for this version
├── LICENSE
├── NOTICE
...
```bash
bash scripts/install_platform.sh initialize --version v4.4.16 --database oracle --edition <community|enterprise> --config config.json
```
...
## v4.4.10 Governance Boundary

The optional model gateway now includes atomic hard/warn quota handling,
encrypted exact replay, immutable usage and financial facts, trusted signed
external evidence, and versioned wallboard publication. Direct model access may
remain enabled and is unknown unless the gateway or a trusted adapter observes
