...
> **v4.4.8 · Community Edition · Oracle**
>
> Database-backed AI Agent infrastructure for Oracle.

![License](https://img.shields.io/badge/License-Apache_2.0-green)

## v4.4.8 Release Highlights

The current release hardens built-in platform Agents and database-authoritative
isolation. It adds a versioned `/platform` command registry, deterministic
command completion and help, a governed maintenance lifecycle, daily
observation proposals, and an explicit Graph Run binding for long-running
maintenance. The Enterprise Compliance Agent remains advisory and proposal-only
with its own private knowledge scope. Oracle Data Grants, PostgreSQL forced
RLS with trusted role mapping, and YashanDB fail-closed privilege revocation
now protect platform private knowledge, commands, maintenance state, and the
isolation inventory.

Graph Runtime core and authorized Graph inspection remain available in the
Production Profile. Manifest draft import, SLO read-only views, and checkpoint
fork are `CONTROLLED` capabilities that require their own authorization and
evidence. MCP 2026-07-28 negotiation and `server/discover`, independent A2A
1.0.1 conformance, replacement-Worker checkpoint/resume and stream
de-duplication, sandbox and approval replay, bounded tool-result compaction,
OTLP Collector delivery, and authorization-filtered GraphRAG projections
remain deferred research. They are not current production claims and are not
enabled by metadata, prompts, Skills, or Agent requests.
...
collaboration groups are traceable collaboration records, not permission grants.
Administrators create a domain with purpose, classification, accountable owner,
and reason, then explicitly confirm each Human or Agent before it may join a
Channel or run in that domain. Existing collaboration groups can produce a
reviewable conversion draft, but historic members and sharing policy never
become authorization automatically.
...
├── RELEASE_NOTES_v4.4.8.md   # release notes for this version
├── LICENSE
├── NOTICE
...
└── scripts/
    ...
    │   ├── connection.py     # oracledb connection layer (adapter)
...
|---------------|-----------------------------------------------|
| Python        | 3.14+                                         |
| Oracle | Oracle AI Database 26ai 23.26.2+                              |
| Driver        | `oracledb>= 4.0.1`       |
...
```bash
bash scripts/install_platform.sh initialize --database oracle --edition <community|enterprise> --config config.json
```
...
PYTHONPATH=scripts "$PYTHON_BIN" -m uvicorn web_app:app --host 0.0.0.0 --port 8001
```

The Chuanxu application is served at `http://<host>:8001/app/monitor`.
