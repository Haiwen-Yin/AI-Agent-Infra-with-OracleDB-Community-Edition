...
Example:
    python3.14 deploy_oracle.py aiadmin oracle 10.10.10.130:1521/ai_agent_ee scripts/deploy/1_schema.sql
"""
import sys
import os
import re
...
def execute_sql_file(conn, sql_file, verbose=True):
    ...
    content = re.sub(r'^EXEC\s+(.+?);?\s*$', r'BEGIN \1; END;', content, flags=re.MULTILINE)
...
def main():
    if len(sys.argv) < 5:
        ...
        print("Example: python3.14 deploy_oracle.py aiadmin oracle 10.10.10.130:1521/ai_agent_ee scripts/deploy/1_schema.sql")
        print("         python3.14 deploy_oracle.py --sysdba sys oracle 10.10.10.130:1521/ai_agent_ee scripts/deploy/4_grants.sql")
        sys.exit(1)
