"""AI Agent Infra v3.4.0 - Connection Pool Tests"""
...
def test_execute_query():
    rows = execute_query("SELECT 1 AS val FROM DUAL")
    assert len(rows) == 1
    assert rows[0]["val"] == 1
...
def test_execute_query_one():
    row = execute_query_one("SELECT 42 AS answer FROM DUAL")
...
def test_execute_dml():
    result = execute("SELECT 1 FROM DUAL")
