...
def test_register_agents():
    ...
    try:
        ...
        execute("INSERT INTO SYSTEM_USERS (USER_ID, USERNAME, PASSWORD_HASH, STATUS, ROLE, AUTH_SOURCE, CREATED_AT, UPDATED_AT) VALUES (:uid, :uname, 'SHA256:placeholder', 'ACTIVE', 'USER', 'LOCAL', SYSTIMESTAMP, SYSTIMESTAMP)", {"uid": USER, "uname": "cred_test_user"})
    except Exception:
        pass
...
def test_register_pool_agent():
    pool_config = {"max_idle_minutes": 60, "skills_tags": ["python", "sql", "oracle"], "auto_wake": False}
...
def test_assign_pool_agent():
    ...
    result = assign_pool_agent(USER, required_skills=["python", "sql"])
    assert result is not None
