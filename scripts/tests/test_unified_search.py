"""AI Agent Infra v3.2.0 - Unified Hybrid Search Tests
...
test("search_unified_basic", lambda: f"results={len(embedding_api.search_unified('vector search optimization', top_k=5))}")

results = embedding_api.search_unified("database partitioning strategies", top_k=5)
test("search_unified_has_scores", lambda: f"scores_keys={list(results[0]['scores'].keys()) if results else 'none'}")

test("search_unified_5_signals", lambda: f"signals={[k for k in results[0]['scores'].keys()] if results else []}")
...
test("fulltext_signal_active", lambda: f"ft_score={ft_score:.3f}")

test("fulltext_search_standalone", lambda: f"results={len(embedding_api.search_fulltext('partitioning', top_k=5))}")

results_ft = embedding_api.search_fulltext("encryption AND credential", top_k=3)
test("fulltext_boolean_query", lambda: f"results={len(results_ft)}")
...
test("relational_domain_filter", lambda: f"has_domain_match={has_domain}")

results_cat = embedding_api.search_unified("performance optimization", top_k=5, category="performance")
test("relational_category_filter", lambda: f"results={len(results_cat)}")
...
test("graph_proximity_active", lambda: f"max_proximity={graph_prox:.3f}")

results_tag = embedding_api.search_unified("vector embedding search", top_k=5, tags=["vector", "search"])
test("tag_filter_works", lambda: f"results={len(results_tag)}")
...
test("cross_type_results", lambda: f"types={sorted(types_present)}")

top_result = results_cross[0] if results_cross else None
test("final_score_computed", lambda: f"score={top_result['final_score']:.4f}" if top_result else "no_results")

test("scores_sum_to_final", lambda: f"weighted_sum={sum(top_result['scores'][k] for k in ['vector','fulltext','relational','graph']):.4f}" if top_result else "no_results")
...
test("multi_filter_combined", lambda: f"results={len(results_multi)}")
...
test("spec_type_search", lambda: f"spec_count={len(spec_results)}")
...
test("edge_count_populated", lambda: f"has_edges={has_edges}")

results_knowledge = embedding_api.search_unified("python oracledb driver", top_k=5, entity_type="KNOWLEDGE")
test("entity_type_filter", lambda: f"all_knowledge={all(r['entity_type'] == 'KNOWLEDGE' for r in results_knowledge)}")
...
test("custom_weights", lambda: f"results={len(results_custom_w)}")
...
test("relational_metadata_joined", lambda: f"has_meta={has_meta}")

results_empty = embedding_api.search_unified("xyzzy_nonexistent_12345", top_k=3)
test("graceful_empty_results", lambda: f"results={len(results_empty)}")

test("search_unified_sql_basic", lambda: f"results={len(embedding_api.search_unified_sql('vector search optimization', top_k=5))}")

results_sql = embedding_api.search_unified_sql("database partitioning strategies", top_k=5)
test("search_unified_sql_has_scores", lambda: f"scores_keys={list(results_sql[0]['scores'].keys()) if results_sql else 'none'}")

test("search_unified_sql_5_signals", lambda: f"signals={[k for k in results_sql[0]['scores'].keys()] if results_sql else []}")

test("search_unified_sql_engine_tag", lambda: f"engine={results_sql[0].get('engine','none') if results_sql else 'none'}")
...
test("search_unified_sql_fulltext_signal", lambda: f"ft_score={ft_sql_score:.3f}")
...
test("search_unified_sql_domain_filter", lambda: f"has_domain_match={has_dom}")
...
if seed_id:
    ...
    test("search_unified_sql_graph_proximity", lambda: f"max_proximity={graph_sql_prox:.3f}")
else:
    test("search_unified_sql_graph_proximity", lambda: "skipped_no_seed")

results_sql_tag = embedding_api.search_unified_sql("vector embedding search", top_k=5, tags=["vector", "search"])
test("search_unified_sql_tag_filter", lambda: f"results={len(results_sql_tag)}")

results_sql_kw = embedding_api.search_unified_sql("oracle database architecture", top_k=5, entity_type="KNOWLEDGE")
test("search_unified_sql_entity_type_filter", lambda: f"all_knowledge={all(r['entity_type'] == 'KNOWLEDGE' for r in results_sql_kw)}")

results_sql_empty = embedding_api.search_unified_sql("xyzzy_nonexistent_12345", top_k=3)
test("search_unified_sql_graceful_empty", lambda: f"results={len(results_sql_empty)}")
...
test("search_unified_sql_custom_weights", lambda: f"results={len(results_sql_custom)}")
