"""Oracle Memory System v2.3.0 - Spec API Tests"""
...
def test_create_spec():
    ...
    assert isinstance(_entity_id, str)
    assert len(_entity_id) > 0
...
def test_link_spec_to_plan():
    ...
    assert isinstance(_plan_id, str)
    assert len(_plan_id) > 0
