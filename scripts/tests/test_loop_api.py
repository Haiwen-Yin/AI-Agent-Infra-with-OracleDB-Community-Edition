"""AI Agent Infra v3.10.0 - Loop Engineering API Tests"""
...
def test_create_loop():
    ...
    print(f"PASS: test_create_loop (id={loop_id[:8]}...)")
    return loop_id
...
def test_start_run():
    ...
    print(f"PASS: test_start_run (run_id={run_id[:8]}...)")
    delete_loop(loop_id)
...
def test_record_iteration():
    ...
    assert isinstance(iter_id, str)
    run = get_run(run_id)
    assert run["iteration_count"] == 1
    assert run["total_tokens"] == 500
    print(f"PASS: test_record_iteration (iter_id={iter_id[:8]}...)")
    delete_loop(loop_id)
