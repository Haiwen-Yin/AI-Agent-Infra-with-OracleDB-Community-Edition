"""AI Agent Infra v4.4.11 - Community Edition - Unified Configuration Manager
...
class MCPConfig:
    ...
    sse_port: int = 9000
    exposed_tools: tuple = (
        "search", "memory_create", "memory_search", "memory_lifecycle_create",
        "memory_lifecycle_chain", "memory_lifecycle_feedback", "memory_lifecycle_candidate",
...
def _decrypt_security_section(sec_raw: dict) -> dict:
    encrypted_blob = sec_raw.get("_encrypted")
    if not encrypted_blob:
        return sec_raw
    from .connection_crypto import decrypt_security_section as _dec
    return _dec(sec_raw)
...
def load_config() -> Config:
    ...
    srv = ServerConfig(
        ...
        session_timeout=int(srv_raw.get("session_timeout") or sec_resolved.get("session_timeout") or os.environ.get("MEMORY_SESSION_TIMEOUT", ServerConfig.session_timeout)),
        node_id=srv_raw.get("node_id") or os.environ.get("MEMORY_SERVER_NODE_ID", ServerConfig.node_id),
    )
    ...
    sec = SecurityConfig(
        masking_enabled=sec_resolved.get("masking_enabled", SecurityConfig.masking_enabled),
        pbkdf2_iterations=int(sec_resolved.get("pbkdf2_iterations", SecurityConfig.pbkdf2_iterations)),
        max_login_attempts=int(sec_resolved.get("max_login_attempts", SecurityConfig.max_login_attempts)),
        lockout_minutes=int(sec_resolved.get("lockout_minutes", SecurityConfig.lockout_minutes)),
    )
