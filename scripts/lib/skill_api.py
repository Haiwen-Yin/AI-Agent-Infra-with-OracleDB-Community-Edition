"""AI Agent Infra v3.10.0 - Skill Storage & Distribution API
...
def register_skill(
    ...
    entity_sql = """
        ...
        VALUES ('ENT_' || RAWTOHEX(SYS_GUID()), 'SKILL', :title, :category, 'ACTIVE',
                :owned_by_agent, :visibility, :workspace_id)
...
def update_skill(skill_id: str, **kwargs: Any) -> bool:
    ...
    if title:
        execute(
            "UPDATE ENTITIES SET TITLE = :vtitle, UPDATED_AT = SYSTIMESTAMP WHERE ENTITY_ID = :veid AND ENTITY_TYPE = 'SKILL'",
    ...
    execute(
        "UPDATE ENTITIES SET UPDATED_AT = SYSTIMESTAMP WHERE ENTITY_ID = :eid AND ENTITY_TYPE = 'SKILL'",
        {"eid": skill_id},
    )
...
def deprecate_skill(skill_id: str) -> bool:
    ...
    execute(
        "UPDATE ENTITIES SET STATUS = 'ARCHIVED', UPDATED_AT = SYSTIMESTAMP WHERE ENTITY_ID = :eid AND ENTITY_TYPE = 'SKILL'",
        {"eid": skill_id},
    )
