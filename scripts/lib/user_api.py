"""AI Agent Infra v3.0.0 - Community Edition - User Management API
...
def register_user(username: str, password: str) -> Optional[Dict[str, Any]]:
    ...
    user_id_sql = "'USR_' || RAWTOHEX(SYS_GUID())"
...
def update_last_login(user_id: str) -> None:
    execute(
        "UPDATE SYSTEM_USERS SET LAST_LOGIN = SYSTIMESTAMP, UPDATED_AT = SYSTIMESTAMP WHERE USER_ID = :v_uid",
        {"v_uid": user_id},
    )
...
def get_user_memories(user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    rows = execute_query(
        """SELECT ENTITY_ID, TITLE, ENTITY_TYPE, STATUS, CREATED_AT
           FROM ENTITIES
           WHERE ENTITY_TYPE = 'MEMORY' AND ROWNUM <= :v_lim
