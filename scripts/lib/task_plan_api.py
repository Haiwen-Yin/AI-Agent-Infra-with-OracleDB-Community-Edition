"""AI Agent Infra v3.4.0 - Community Edition - Task Plan API
...
def update_plan(plan_id: str, **kwargs: Any) -> bool:
    """Update plan fields. Sets COMPLETED_AT on terminal status transitions."""
    valid = {k: v for k, v in kwargs.items() if k in _ALLOWED_PLAN_UPDATES}
    if not valid:
        return False

    if "status" in valid and valid["status"] in _TERMINAL_STATUSES:
        valid["completed_at"] = "SYSTIMESTAMP"
    ...
    sql = f"UPDATE TASK_PLANS SET {set_clause}, UPDATED_AT = SYSTIMESTAMP WHERE PLAN_ID = :plan_id"
...
def update_step(step_id: str, **kwargs: Any) -> bool:
    """Update step fields. Sets timestamps on status transitions."""
...
def add_dependency(source_plan_id: str, target_plan_id: str, dep_type: str) -> str:
    ...
    sql = """
        ...
        VALUES ('DEP_' || RAWTOHEX(SYS_GUID()), :source, :target, :dep_type)
...
def log_tool_call(plan_id: str, step_id: Optional[str] = None,
    ...
    sql = """
        ...
        VALUES ('CALL_' || RAWTOHEX(SYS_GUID()), :plan_id, :step_id, :tool_name,
                :tool_input, :tool_output, :status, :duration_ms)
...
def save_snapshot(plan_id: str, snapshot_type: str, context_data: Any) -> str:
    ...
    sql = """
        ...
        VALUES ('SNAP_' || RAWTOHEX(SYS_GUID()), :plan_id, :snapshot_type, :context_data)
