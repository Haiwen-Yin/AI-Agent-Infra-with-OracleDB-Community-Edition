"""AI Agent Infra v3.0.0 - Community Edition - Skill Token API"""
...
def request_skill_access(agent_id: str, session_id: str, skill_id: str) -> Optional[Dict]:
    ...
    token_sql = """
        ...
        VALUES ('TKN_' || RAWTOHEX(SYS_GUID()), :skill_id, :agent_id, :session_id,
                TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') + NUMTODSINTERVAL(:expires_at, 'SECOND'),
                'N')
    ...
    execute(
        """UPDATE SKILL_ACCESS_TOKEN
           SET IS_CONSUMED = 'Y',
               ...
               DOWNLOAD_EXPIRES_AT = TO_TIMESTAMP('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') 
                                     + NUMTODSINTERVAL(:dl_expires, 'SECOND')
           WHERE TOKEN_ID = :tid""",
        {"tid": token_id, "dl_token": download_token, "dl_expires": download_expires},
    )
