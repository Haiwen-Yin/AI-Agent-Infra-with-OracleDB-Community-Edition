#!/bin/bash
# ============================================================================
# AI Agent Infra v3.2.0 - Community Edition - Web Server Control Script
...
do_status() {
    load_env
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  AI Agent Infra v3.2.0 - Community${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo ""
    if is_running; then
        local pid=$(get_pid)
...
do_start() {
    ...
    if [ -z "$PYTHON" ]; then
        ...
        echo -e "${YELLOW}Install oracledb-compatible Python (3.8+) and update script${NC}"
        exit 1
    fi

    if ! $PYTHON -c "import oracledb" 2>/dev/null; then
        echo -e "${YELLOW}oracledb not installed for $PYTHON, installing...${NC}"
        $PYTHON -m pip install oracledb -q 2>/dev/null || {
            echo -e "${RED}Failed to install oracledb${NC}"; exit 1
        }
    fi
...
case "${1:-}" in
    ...
    log)     do_log ;;
    *)
        echo "AI Agent Infra v3.2.0 - Community Edition - Web Server Control"
        echo ""
