#!/usr/bin/env bash
# Chuanxu v4.3.1 - AI Agent Infra v4.3.1 - {{EDITION_LABEL}} unified FastAPI/Uvicorn server controller.
...
export AI_AGENT_RELEASE_DATE="2026-07-31"
...
start_server() {
  ...
  if ! "$PYTHON" -c 'import fastapi, uvicorn' >/dev/null 2>&1; then
    echo "FastAPI and Uvicorn are required. Install requirements.txt with the approved Python 3.14 interpreter before starting." >&2
    exit 1
  fi
  echo "Starting Chuanxu v4.3.1 ($DB_DIALECT, $PROFILE) on $HOST:$PORT"
